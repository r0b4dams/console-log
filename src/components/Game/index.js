    ...
    return (
      <>
        <Link to={`/GamePage/${game.id}`} className={location.pathname === "/GamePage" ? "nav-link active" : "nav-link"}>
          <article className="grid grid-flow-col bg-cover gameCard p-1 flex space-x-4 mb-2 sm:mx-8 rounded hover:bg-gray-300 border-2" style={{
            backgroundImage: `url(${game.background_image})`
          }}>
            {/* relative flex-auto bg-gray-200 bg-opacity-80 rounded px-1 grid grid-rows-1 grid-flow-col */}

            <div className="p-1 flex space-x-4 bg-gray-200 bg-opacity-80 rounded">
              ...
              <div className="min-w-0 relative flex-auto sm:pr-20 lg:pr-0 xl:pr-20">
                ...
                {game.playtime > 0 &&
                      <div>
                        <dt className="sr-only">Play Times</dt>
